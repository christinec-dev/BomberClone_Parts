...
# ---------------- AI Path Generation -------------------------------------	
# Custom hash function for Vector2i
func hash_vector2i(vec):
	return vec.x + vec.y * 10000  # You can adjust the multiplier based on your map size

#Initializes astar points & draws it for debugging
func initialize_astar():
	for x in range(map_width):
		for y in range(map_height):
			var cell_coords = Vector2i(x, y + map_offset)
			if is_cell_walkable(cell_coords):
				astar.add_point(hash_vector2i(cell_coords), cell_coords)
	connect_astar_points()
	NOTIFICATION_DRAW
	
# Checks if a cell is walkable (empty or breakable)
func is_cell_walkable(cell_coords):
	return is_cell_empty(2, cell_coords)
	
# Connects A* points (only orthogonal neighbors)
func connect_astar_points():
	for x in range(map_width):
		for y in range(map_height):
			var cell_coords = Vector2i(x, y + map_offset)
			if astar.has_point(hash_vector2i(cell_coords)):
				for dx in [-1, 0, 1]:
					for dy in [-1, 0, 1]:
						if dx == 0 and dy == 0:
							continue
						...
						if astar.has_point(hash_vector2i(neighbor_coords)):
							var cost = 1  # Cost is 1 for all neighbors now
							astar.connect_points(hash_vector2i(cell_coords), hash_vector2i(neighbor_coords), cost)

# Gets a path from start to end using A* algorithm
func find_path(start: Vector2, end: Vector2) -> Array:
	var start_converted = Vector2(int(start.x / 16), int(start.y / 16))
	var end_converted = Vector2(int(end.x / 16), int(end.y / 16))
	var start_id = hash_vector2i(start_converted)
	var end_id = hash_vector2i(end_converted)
...
func _draw():
	for point_id in astar.get_point_ids():
		...
		var screen_point = tilemap.map_to_local(point)  # Convert to screen coordinates
		for neighbor_id in astar.get_point_connections(point_id):
			...
			draw_line(screen_point, screen_neighbor_point, Color(0, 0, 1), 1)  # Draw line in green with a width of 1
